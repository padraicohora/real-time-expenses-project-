...
summary_data$Mode <- factor(summary_data$Mode, levels = summary_data$Mode)

chart_1 <- summary_data %>% ggplot(aes(x = Mode, y = total_amount_sum, col = Mode, fill=Mode)) +
...
summary_data$Mode <- factor(summary_data_2$Mode, levels = summary_data_2$Mode)

chart_2 <-summary_data_2 %>% ggplot(aes(x = Mode, y = total_amount_count, col = Mode, fill=Mode)) +
  geom_bar(stat="identity", alpha=.6, width=.4) +
  coord_flip() +
...
grid.arrange(chart_1, chart_2,nrow = 2)

```
#### Chart analysis
Chart shows Savings bank account has the highest largest sum of Amount Transactions
