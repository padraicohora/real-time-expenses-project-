...
weekly_expense_ts <- ts(weekly_expense$Total_Expense, start = c(start_year, start_month), frequency = 52)
```
...
actual_values <- weekly_expense$Total_Expense
predicted_values <- fitted(weekly_model)
...
# Calculate MAE and RMSE
actual_values <- weekly_expense$Total_Expense
predicted_values <- fitted(simplified_model)
mae <- mean(abs(actual_values - predicted_values))
rmse <- sqrt(mean((actual_values - predicted_values)^2))
list(MAE = mae, RMSE = rmse)
```
...
## ARIMA model 
```{r}
# Assuming weekly_expense_ts is your time series data
# Fit an ARIMA model
...
forecasted_values <- forecast(arima_model, h = 52)  # Forecasting 10 periods ahead
...
combined_data <- bind_cols(data, encoded_data)
str(combined_data)
